...
CREATE OR REPLACE FUNCTION usuarios.put_documento_usuario(
...
BEGIN
	RETURN QUERY
	UPDATE usuarios.tbl_cat_usuario
	SET 
		nombre = CASE WHEN str_nombre <> '' THEN str_nombre ELSE nombre END,
		id_ad = CASE WHEN str_id_ad <> '' THEN str_id_ad ELSE id_ad END,
		correo = CASE WHEN str_correo <> '' THEN str_correo ELSE correo END,
		user_principal_name = CASE WHEN str_user_principal_name <> '' THEN str_user_principal_name ELSE user_principal_name END,
		telefono = CASE WHEN int_telefono IS NOT NULL THEN int_telefono ELSE telefono END,
		codigo_pais = CASE WHEN int_codigo_pais IS NOT NULL THEN int_codigo_pais ELSE codigo_pais END,
		id_cat_usuario_dominio = CASE WHEN int_id_cat_usuario_dominio IS NOT NULL THEN int_id_cat_usuario_dominio ELSE id_cat_usuario_dominio END,
		tipo_usuario = CASE WHEN int_tipo_usuario IS NOT NULL THEN int_tipo_usuario ELSE tipo_usuario END,
		id_cat_usuario_categoria = CASE WHEN int_id_cat_usuario_categoria IS NOT NULL THEN int_id_cat_usuario_categoria ELSE id_cat_usuario_categoria END,
		username = CASE WHEN str_username <> '' THEN str_username ELSE username END,
    ...
    WHERE id_cat_usuario = int_id_cat_usuario
