...
CREATE
OR REPLACE FUNCTION recepciones_documento.post_documento_estadoasignacioncanal(
  int_id_cat_documento_estado integer,
  int_id_cat_documento_canal integer,
  numeric_progreso numeric,
  int_id_creador integer
) RETURNS TABLE(id_asignacion_canal integer) LANGUAGE 'plpgsql' COST 100 VOLATILE PARALLEL UNSAFE ROWS 1000 AS $ BODY $ BEGIN RETURN QUERY
INSERT INTO
  recepciones_documento.tbl_det_documento_estado_asignacion_canal(
...
ALTER FUNCTION recepciones_documento.post_documento_estadoasignacioncanal(integer, integer, numeric, integer) OWNER TO vince;