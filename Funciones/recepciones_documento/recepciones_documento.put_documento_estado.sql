...
CREATE
OR REPLACE FUNCTION recepciones_documento.put_documento_estado(
...
) RETURNS TABLE(id_estado integer) LANGUAGE 'plpgsql' COST 100 VOLATILE PARALLEL UNSAFE ROWS 1000 AS $ BODY $ BEGIN RETURN QUERY
UPDATE
  recepciones_documento.tbl_cat_documento_estado
SET
  nombre = CASE
  ...
  END,
  estado_final = CASE
    WHEN int_estado_final IS NOT NULL THEN int_estado_final
    ELSE estado_final
  END,
  estado = CASE
...
WHERE
  id_cat_documento_estado = int_id_cat_documento_estado RETURNING recepciones_documento.tbl_cat_documento_estado.id_cat_documento_estado;

END;

$ BODY $;

ALTER FUNCTION recepciones_documento.put_documento_estado(
  integer,
  character varying,
  character varying,
  integer,
